...
class RagClient:
    def init(self):
        ...
        self.open_ai_token = os.environ['OPENAI_API_KEY']
        self.load_embeddings()

    def load_embeddings(self):
        chunks_from_file=[]
        self.splitter = CharacterTextSplitter(chunk_size=1000, chunk_overlap=10)
        ...
        self.chunk_embeddings=[self.model.encode(chunk) for chunk in chunks]  # Use self to reference class variables
    ...
    def answer(self, text):
        ...
        retrieved_list = [self.chunk_embeddings[i] for i in I[0]]
        retrieved_object_list = []
        for i in I[0]: 
            retrieved_object_list.append({
                    "content": self.chunk_embeddings[i]
                })
